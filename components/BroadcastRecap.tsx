...
export function BroadcastRecap({ recap }: { recap: BroadcastRecapData }) {
  ...
  useEffect(() => {
    ...
    if (characters < message.length) {
      const timer = window.setTimeout(() => {
        ...
        if (characters % 2 === 0 && audio.current && !muted) chatter(audio.current);
  ...
  }, [characters, line, muted, open, recap.lines]);
  ...
  return (
    <div className="recap-overlay" role="dialog" aria-modal="true" aria-label="This Week in Mortal Locks">
      <button className="recap-backdrop" aria-label="Close weekly recap" onClick={() => setOpen(false)} />
      <section className="recap-console">
        <div className="recap-toolbar"><span>{recap.eyebrow}</span><div><button onClick={() => { const nextMuted = !muted; setMuted(nextMuted); if (!nextMuted) ensureAudio(); }}>{muted ? 'SOUND OFF' : 'SOUND ON'}</button><button onClick={() => setOpen(false)}>END BROADCAST ×</button></div></div>
        ...
        <div className="recap-controls">
          ...
          {!muted && <button onClick={() => { const context = ensureAudio(); if (context) startTheme(context); }}>♪ PLAY STINGER</button>}
          {finished && <button onClick={() => setOpen(false)}>BACK TO THE BOARD →</button>}
        </div>
      </section>
    </div>
  );
